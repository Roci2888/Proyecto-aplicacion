...
class AuthControllerTest {
    ...
    /**
     * Prueba que logout invalide la sesión y redirija a login
     */
    @Test
    void logout_ShouldInvalidateSessionAndRedirectToLogin() throws Exception {
        ...
        // La sesión debe quedar invalidada: acceder de nuevo a un recurso protegido
        // con la misma referencia de sesión ya no debería reconocer al usuario.
        Assertions.assertThrows(IllegalStateException.class, () -> {
            session.getAttribute("usuario"); // MockHttpSession lanza excepción tras invalidate()
        });
    }

    /**
     * Prueba el camino REAL de duplicidad: existsByUsername() devuelve true
     * antes de intentar guardar (así es como el controlador realmente decide)
     */
    @Test
    void registerUser_ShouldRedirectToRegister_WhenExistsByUsernameReturnsTrue() throws Exception {
        ...
        mockMvc.perform(post("/auth/register")
                ...
                .andExpect(flash().attribute("error",
                        "El usuario 'existinguser' ya existe. Por favor elige otro nombre."));

        // save() NUNCA debe llamarse si el username ya existe
        verify(userService, never()).save(any(User.class));
        verify(userService, times(1)).existsByUsername("existinguser");
    }

    /**
     * Prueba que si existsByUsername() lanza una excepción inesperada,
     * el registro caiga en el catch general
     */
    @Test
    void registerUser_ShouldHandleException_WhenExistsByUsernameThrows() throws Exception {
