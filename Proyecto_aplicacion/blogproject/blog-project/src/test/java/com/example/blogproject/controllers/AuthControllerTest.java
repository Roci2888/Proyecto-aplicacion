...
class AuthControllerTest {
    ...
    void logout_ShouldInvalidateSessionAndRedirectToLogin() throws Exception {
        ...
        // Verifica que la sesión fue invalidada (el atributo ya no existe)
    ...
    /**
     * Prueba que se maneje correctamente una excepción al eliminar usuario
     */
    @Test
    void deleteUser_ShouldHandleDeleteException() throws Exception {
        session.setAttribute("usuario", adminUser);
        // Simula que el servicio lanza excepción al eliminar
        doThrow(new RuntimeException("Error deleting user")).when(userService).deleteUser("999");

        mockMvc.perform(post("/admin/delete/999").session(session))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/admin")); // Aún así redirige al admin
